...
def apply_distortion(im_cv, scale_x, scale_y, amount, center_x, center_y):
    (h, w, _) = im_cv.shape
...
# Function to find face landmarks and get the center of the face
def get_face_center(image):
    ...
    if len(rects) > 0:
        ...
        center_x = np.mean(shape_np[:, 0])
        center_y = np.mean(shape_np[:, 1])
        return center_x, center_y
    else:
        return image.shape[1] // 2, image.shape[0] // 2

# Data for clinic rankings
data = {
...
if page == "画像歪み調整＆カメラ":
    ...
    if uploaded_file is not None:
        ...
        # Get face center
        center_x, center_y = get_face_center(im_cv)
        ...
        # Apply distortion
        distorted_image = apply_distortion(im_cv, scale_x, scale_y, amount, center_x, center_y)
